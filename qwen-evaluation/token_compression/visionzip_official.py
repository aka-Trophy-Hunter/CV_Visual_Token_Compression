...
class Qwen2_5_VLVisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa") -> None:
        ...
        # print('VL attn_implementation:', attn_implementation)
...
class Qwen2_5_VLForConditionalGeneration_VisionZip(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        #     dominant_num = int((self.budget-0.05) * attn_logits.size(0))
        ...
        #     inputs_embeds[:,contexual_input_idx] =  contextual_tokens.to(dtype=inputs_embeds.dtype)
        #     inputs_embeds = inputs_embeds[:, img_mask]
        #     del contextual_tokens, hidden_states_filtered, hidden_to_merge,aggregated_hidden

        # ==================================>>>>> MODIFICATION START <<<<<==================================
        if select_pixel:
            # self.budget is not defined in the original code, assuming it's a class attribute like 0.5
            if not hasattr(self, 'budget'):
                 self.budget = 0.5 # Example value, please set it correctly.
            ...
            # `mask` identifies dominant tokens within the image token group
            mask = torch.zeros_like(attn_logits, dtype=torch.bool)
            mask[topk_indices] = True
            
            # `contextual_mask` identifies non-dominant tokens for further processing
            contextual_mask = ~mask
            metric_filtered = attn_key[:, contextual_mask]
            metric_normalized = metric_filtered / metric_filtered.norm(dim=-1, keepdim=True)
            del attn_key, metric_filtered
            ...
            # `select_mask` will be True for all image tokens we want to KEEP (dominant + contextual targets)
            select_mask = torch.zeros_like(attn_logits, dtype=torch.bool)
            select_mask[topk_indices] = True
            
            # `false_pos` are the indices of non-dominant tokens within the image token group
            false_pos = (~select_mask).nonzero(as_tuple=True)[0]
            # Update `select_mask` to also keep the contextual target tokens
            select_mask[false_pos[target_indices]] = True
 

            # Find where the image tokens are in the full sequence
            img_token_mask = (input_ids == self.config.image_token_id)[0]
            st_idx = torch.nonzero(img_token_mask, as_tuple=True)[0]

            if st_idx.numel() > 0:
                # This block now handles all filtering and merging logic
                
                # FIX 1: Correctly get all image embeddings, even if they are not contiguous
                all_image_embeds = inputs_embeds[:, st_idx]

                # FIX 2: Use the correctly gathered embeddings to get non-dominant ones for merging
                hidden_states_filtered = all_image_embeds[:, contextual_mask]
                ...
                # Get the indices of contextual tokens in the *original full sequence*
                contexual_input_idx = st_idx[false_pos[target_indices]]

                # Create the final mask for keeping tokens.
                # It keeps all text tokens and the selected image tokens.
                final_keep_mask = ~img_token_mask
                final_keep_mask[st_idx] = select_mask

                # Update the embeddings for contextual tokens *before* filtering the sequence
                inputs_embeds[:, contexual_input_idx] = contextual_tokens.to(dtype=inputs_embeds.dtype)

                # Filter all relevant tensors based on the final mask
                position_ids = position_ids[:, :, final_keep_mask]
                if attention_mask is not None:
                    attention_mask = attention_mask[:, final_keep_mask]
                if labels is not None:
                    labels = labels[:, final_keep_mask]
                inputs_embeds = inputs_embeds[:, final_keep_mask]
