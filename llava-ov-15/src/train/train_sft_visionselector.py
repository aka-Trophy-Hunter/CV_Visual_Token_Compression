...
from compression_method.selector_scorer import *
import types
import random
import numpy as np
...
class ScheduledWeightTrainer(QwenSFTTrainer):
...
def configure_vision_tower(model, training_args, compute_dtype, device):
    ...
    # add 
    selector_params = model.visual.importance_scorer.parameters()
    set_requires_grad(selector_params, not training_args.freeze_selector)
    ...
    if not training_args.freeze_selector:
        rank0_print("Selector parameters are set to trainable")
    else:
        rank0_print("Selector parameters are frozen")
...
def train():
    ...
    try:
        if "Qwen2.5" in model_args.model_id:
            ...
            model.forward = types.MethodType(llavaov15_generation_forward_selector, model)
            model.model.forward = types.MethodType(llavaov15_vlmodel_forward_selector, model.model)
    ...
    if len(trainable_param_names) > 0:
        rank0_print(f"Trainable params: {trainable_param_names}")
