...
class RiceBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "sdpa", layer_idx: int = -1) -> None:
        ...
        self.attn = RICE_ATTENTION_CLASSES[attn_implementation](
            config.hidden_size, num_heads=config.num_heads, layer_idx=layer_idx
        )
...
class RiceTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.blocks = nn.ModuleList(
            [RiceBlock(config, config._attn_implementation, layer_idx=i) for i in range(config.depth)]
        )
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                text_image_mask = (input_ids != self.config.image_token_id)   # HACK  change here to get image_mask(image position is false, else is true)
                self.language_model.text_image_mask = text_image_mask
...
class LLaVAOneVision1_5_ForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        outputs, visual_token_num = self.model(
            input_ids=input_ids,
